...
# Global in-memory cache for storing summarized text keyed by the input text.
cache = {}

# API Keys integrated directly into the code (not recommended for production).
NEWS_API_KEY = "488ced06fd3740af8bbcc87364b978af"
GEMINI_API_KEY = "AIzaSyBeIDgECGoAVvVf_5X7vjWK7j8CTdf6Hi0"

# Configure Google Gemini AI with your API key.
genai.configure(api_key=GEMINI_API_KEY)

# Load Hugging Face Summarization Model (using facebook/bart-large-cnn).
...
def summarize_with_huggingface(text):
    ...
    """
    try:
        # Limit input to avoid overloading the model.
...
async def process_article(article: dict):
    ...
    # Offload the CPU-bound summarization to a separate thread.
    summary = await asyncio.to_thread(summarize_text, full_text)
    
    # Even if summary isn't available, we return basic data.
    return {
        "title": title,
...
async def get_basic_news():
    ...
    url = (
        ...
        f"sources=bbc-news,cnn,associated-press,the-new-york-times,the-guardian-uk&"
        f"language=en&pageSize=50&apiKey={NEWS_API_KEY}"
    )
